...
def pix_to_xyz(pixel, height, bounds, pixel_size, skip_height=False):
  ...
  if not skip_height:
    ...
    # z = bounds[2, 0] + 0.1  # TODO: This is hardcoded height due to wrong height values from low resolution height maps
  else:
    z = 0.0
  return (x, y, z)
...
def sample_distribution(prob, n_samples=1):
  ...
  rand_ind_coords = np.array(np.unravel_index(rand_ind, prob.shape))
...
def depth_pix_to_xyz(pixel, d, intrinsics, pose_matrix):
  u0 = intrinsics[0, 2]
  v0 = intrinsics[1, 2]
  fu = intrinsics[0, 0]
  fv = intrinsics[1, 1]
  ...
  xyz_world = np.matmul(pose_matrix, xyz_camera)
  return xyz_world[:3] / xyz_world[3]
...
def map_dict_over_poses(trg_dict, src_dict, f):
  for pose_key in [POSETYPE.POSE0, POSETYPE.POSE1]:
    trg_dict[pose_key] = f(src_dict[pose_key])
...
def np_unknown_cat(acc, arr):
  ...
  if acc is None:
    ...
    return np.concatenate((acc, arrE), axis=0)

def torch_unknown_cat(acc, arr):
  arrE = arr.unsqueeze(dim=0)
  if acc is None:
    return arrE
  else:
    return torch.cat((acc, arrE), dim=0)